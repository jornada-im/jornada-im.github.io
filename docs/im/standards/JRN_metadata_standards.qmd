...
This document defines metadata standards to use when creating or revising EML files for Jornada Basin LTER research data packages. These standards are under active development as we revamp our data catalog, but they are essentially derived from the ["Best Practices for Dataset Metadata in Ecological Metadata Language (EML)"](https://ediorg.github.io/data-package-best-practices/guide-eml-bp/) guide from the LTER Network and EDI,with modifications to suit the Jornada Basin LTER and its researchers.
