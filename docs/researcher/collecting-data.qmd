...
date: last-modified
from: markdown+emoji
#mermaid-format: png # svg - this allows rendering of mermaid in advance
---
...
* Remember to document the observational units (subject of the measured/observed quantities) for each variable, i.e. the mass of _the captured individuals_, or the temperature of _soil at 5 centimeter depth in plot #4_. 
...
* Again, remember to document the observational unit (what the categorical labels apply to) for each variable, i.e. the species code of _the captured individuals_, or the responses _to question n_. 
...
* The observational unit in most cases is _the time each observation or measurement occurred_.
...
* This is a very diverse category of data types. Check the [Special cases and unstructured data](#special-cases-and-unstructured-data) section for more information and ideas about what is important.
...
1. Know what your data are about, including the variables observed or measured, and the observational units.
...
Keep in mind that wide/untidy format isn't always bad. Wide tables tend to take up less space on disk, and in addition, they are called for in many standard statistical analyses. We'll talk a little more about this in the [Working with data](data-work.qmd) chapter.
...
|               | - Genome, metagenome, transcriptome, metabolome and     |
|               |   other "omics" studies                                 |
+---------------+---------------------------------------------------------+
| Content       | - Nucleotide reads from amplicons (such as 16S rRNA),   |
|               |   genomes or metagenomes                                |
|               | - Feature tables (ASVs, OTUs per sample) derived from   |
|               |   bioinformatics of amplicon or metabarcoding reads.    |
|               | - Taxonomy tables                                       |
+---------------+---------------------------------------------------------+
...
|               | - CSV tables for feature and taxonomy tables            |
|               | - ???                                                   |
+---------------+---------------------------------------------------------+
...
1. Always preserve the raw data in its original form. Chances are you’ll want to go back to the raw source data at least once.
2. Use computer code or another scripted workflow, such as R or Python scripts,[^4][^5] or OpenRefine[^6], to clean and QA/QC your raw data, not manual editing. See the guidance for doing this reproducibly in the [Working with data](data-work.qmd#reproducibility) chapter.
3. If you can, use numerical or statistical rules to find and remove error values or outliers, not "judgment calls" or "eyeballing it."
4. Spread the job around! Data cleaning typically demands a huge fraction of the total time devoted to working with data [@wickham_tidy_2014] and it can be tedious work. Sharing the data-cleaning burden is more equitable, and it gets more eyes on the data which will improve data quality.

:::

[^4]: In R, the `tidyverse` libraries (e.g., `dplyr`, `tidyr`, `stringr`) are often used for data cleaning, as are additional libraries like [`janitor`](https://sfirke.github.io/janitor/).
[^5]: In Python, `pandas` and `numpy` libraries provide useful data cleaning features. There are also some stand-alone cleaning tools like [`pyjanitor`](https://pyjanitor-devs.github.io/pyjanitor/) (started as a re-implementation of the R version) and [`cleanlab`](https://docs.cleanlab.ai/stable/index.html) (geared towards machine learning applications).
[^6]: [OpenRefine](https://openrefine.org/) is an open-source, cross-platform tool for iterative, scripted data cleaning.

### Data processing levels

Consider using the concept of data processing “levels,” meaning that defined sets of data cleaning and QA/QC operations are applied to the data consistently and in a stepwise fashion, and resulting data files are labeled distinctly and stored separately. Managing your data this way during the data collection phase sets your project on a path to being well-organized and reproducible later in the analysis phase (see the [Working with data](data-work.qmd) chapter). There are different ways to apply the concept of processing levels to your data.
...
- Both the R and Python data science ecosystems have excellent documentation resources that thoroughly cover data cleaning. For R, consider starting with Hadley Wickham's _R for Data Science_ book chapter on [data tidying](https://r4ds.hadley.nz/data-tidy) [@wickham_r_2023], and for Python check Wes McKinney's _Python for Data Analysis_ book chapter on [data cleaning and preparation](https://wesmckinney.com/book/data-cleaning) [@mckinney_python_2022].
...
[^7]:  There is a summary of this from CISA.gov [here](https://www.cisa.gov/sites/default/files/publications/data_backup_options.pdf).
