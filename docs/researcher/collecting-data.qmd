...
2. Use computer code or another scripted workflow, such as R or Python scripts,[^3][^4] or OpenRefine[^5], to clean and QA/QC your raw data, not manual editing. See the guidance for doing this reproducibly in the [Working with data](data-work.qmd#reproducibility) chapter.
...
[^3]: In R, the `tidyverse` libraries (e.g., `dplyr`, `tidyr`, `stringr`) are often used for data cleaning, as are additional libraries like [`janitor`](https://sfirke.github.io/janitor/).
[^4]: In Python, `pandas` and `numpy` libraries provide useful data cleaning features. There are also some stand-alone cleaning tools like [`pyjanitor`](https://pyjanitor-devs.github.io/pyjanitor/) (started as a re-implementation of the R version) and [`cleanlab`](https://docs.cleanlab.ai/stable/index.html) (geared towards machine learning applications).
[^5]: [OpenRefine](https://openrefine.org/) is an open-source, cross-platform tool for iterative, scripted data cleaning.
