...
Those who use the data files you share (including your future self) will need some cues on how to do that, so your metadata should describe each file as clearly as possible. In the previous chapter there are some [file format recommendations](./collecting-data.qmd#file-formats) that we won't repeat here except to say that using file formats that are as open (non-proprietary) and understandable as possible, and describing them clearly and thoroughly, will benefit everyone involved.
...
The data in the files you share don't speak for themselves, so metadata must explain them well enough that others can understand and use each data file. The previous chapter describes the important aspects of [data content](./collecting-data.qmd#content), and provides [data formatting recommendations](./collecting-data.qmd#data-format) to consider. Below we describe how to communicate about the content, formatting, and meaning of data files with **metadata attributes that should be provided for _each variable_ in a data file**. In tabular data files, the individual variables are usually represented as columns, but other arrangements, such as bands/layers in raster images, are not uncommon.
