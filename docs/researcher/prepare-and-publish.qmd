...
[^4]: The EDI repository has useful resources on [data quality control]({{< var url.edi >}}/resources/cleaning-data-and-quality-control), [dataset design]({{< var url.edi >}}/resources/designing-a-data-package), and [metadata preparation]({{< var url.edi >}}/resources/creating-metadata-for-publication).
...
There are many data repositories available to researchers now [^5], and choosing one is not necessarily easy. Make sure the repository you choose meets these essential criteria. 
...
[^5]: The Registry of Research Data Repositories ([re3data.org](https://re3data.org)) project is a comprehensive tracker of information about research data repositories.
...
Assembling a dataset for publication means putting metadata together with your data files. Ideally, the metadata should be in a format that provides rich detail and is both human- and machine-readable, and we recommend using EML for most Jornada datasets. Two methods to collect valid and complete metadata for most datasets are described below, **metadata templates** and the **ezEML app**, but if you are sure you won't be publishing in the EDI repository or using the EML metadata standard, start with the templates and contact a Jornada data manager for further help. 

### Metadata templates

A metadata template is a document with a structure and cues that help you collect the essential metadata needed to describe a published dataset. We have created Jornada metadata templates in Microsoft Word (.docx) and Excel (.xlsx) formats that are available on the [downloads page](../../downloads/index.qmd). These templates contain sections for all the critical pieces of metadata we described in the [last chapter](metadata.qmd#recommendations-for-jornada-metadata), and helpful instructions on what to include and how to structure the information. The Excel version is slightly more detailed and may be useful for complex datasets. Once you start describing your data, **keep your metadata template together with the data files you intend to publish.**

Keep in mind that these templates are fairly general in nature and can be used as a starting place for any kind of metadata destined for any repository. However, the amount of documentation and interactive assistance you get with a template is less than what you get in the ezEML web app described below. Completed templates and accompanying data files should be sent to the Jornada IM team ({{< var email.jrn_im >}}).
...
The EDI repository has created a web app called [ezEML]({{< var url.ezeml >}}) for describing research datasets and creating a standardized metadata documents for publication. The ezEML app creates metadata documents in the Ecological Metadata Language, or EML, which is a dialect of XML. The tool simplifies most of the complexity of EML and XML, and is an easy method to author well-documented datasets. There is a **Jornada EML template** available on the site, so the recommended process for Jornada researchers is:
  ...
  2. Start a new EML dataset using the **EML Documents > New from Template** menu item, as shown in @fig-jrntemplate.
  3. Navigate to and select the **LTER/JRN/JRN_template_general** template to create a new EML document from the JRN template (also shown in @fig-jrntemplate).
      - This template is pre-populated with Jornada metadata, including contact information, geographic coverage, and more. You can edit this metadata as you see fit, but we recommend keeping contact information for the data manager and current funding awards.
      - If your dataset is not associated with the Jornada, you do not need to use a template and can select **EML Documents > New**
  4. Give the dataset a unique name.
      - You can save your metadata and then return to a saved EML document anytime.
  5. Follow the sequence of forms in the menu at left, and ezEML's prompts, to upload data files and enter metadata for your dataset. Each section of your metadata will have help available ("?" icons) and some fields will already be filled if you are using the JRN template.
      - Note that the colored badges turn from red, to yellow, to green as you complete the metadata. 
      - You won't always need every element of metadata to be completed ("Other entity", for example, is optional).
  6. Use the **Check metadata** and **Check data tables** tools at the bottom left to check the completeness and validity of your dataset.
      - Green badges next to metadata mean your dataset is well described and ready to share (and yellow or red can indicate missing values or errors).
  7. When ready, click **Submit/Share Package** and then **Collaborate with Colleagues**. 
      - *DO NOT USE **Submit Package to EDI** or we may miss your dataset*.
  ...
  At this point, the Jornada IM Team will receive a notification and can access your dataset in ezEML to review, edit, and publish to EDI. It never hurts to send a reminder in case the data managers miss this EDI notification. Keep in mind that you can start an EML document in ezEML and then save it and continue later. Just be sure to use the same login as before.

![The "JRN Template - General" EML template can be found in the ezEML app and is a great way to get started preparing the metadata for a new Jornada dataset.](./img/jrn_template_ezeml.png){width=100% #fig-jrntemplate}

## The review process

Once you have submitted the dataset to the Jornada IM team, the data and metadata are securely archived and a round of review and editing begins (see @fig-jornadapublish). A data manager will review the metadata for completeness, compliance with Jornada standards, and mismatches between metadata and data. If revisions are needed or errors are found you will receive an email requesting the changes. You can submit your revisions back to the IM team in an updated template document, or by making the edits in ezEML, as appropriate.
